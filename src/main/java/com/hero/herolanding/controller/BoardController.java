	...
	public String Board_write(BoardDTO dto, BindingResult bindingResult, HttpServletRequest request) {
		...
		if (!StringUtils.hasText(dto.getPost_title())) {
			bindingResult.addError(
					new FieldError("dto", "post_title", dto.getPost_title(), false, null, null, "제목은 필수입니다."));
		} // 제목 검사 ( 값을 입력했는지 검사 )

		if (!StringUtils.hasText(dto.getPost_content())) {
			bindingResult.addError(
					new FieldError("dto", "post_content", dto.getPost_content(), false, null, null, "내용은 필수입니다."));
		} // 내용 검사 ( 값을 입력했는지 검사 )
		...
		if (dto.getWriteType() == null) {
			bindingResult
					.addError(new FieldError("dto", "writeType", dto.getWriteType(), false, null, null, "게시판을 선택해주세요"));
		} // 게시판 선택했는지 검사

		if (bindingResult.hasErrors())
			return "board/board_write"; // 에러가 있는 경우 입력폼으로 다시 이동
		...
		board.setInsertDate(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))); // 지금 시간을
																												// 포맷해서
																												// 넣어줌
...
//===================== 리스트 불러오기 ===============================
	...
	public String board_list(@PathVariable("id") Integer page, Model model, HttpServletRequest request) {
		...
		// 현재 페이지를 보여주기위한 변수 etc: 14번쨰에 있다면 11 , 12 , 13 , 14 , 15 , 16 , 17 , 18, 19
		// 를보여줘야하고
		// etc: 22번쨰에 있다면 21 , 22 , 23 ...

		if ((page / 10) * 10 + 10 < (boardService.BoardCount().size() / 10) + 1) // 현제 보고있는 페이지보다 게시물이 100개 이상 있는 경우는 모두
																					// 보여줌
		{
			model.addAttribute("last", 9);
		} else {
			model.addAttribute("last", ((boardService.BoardCount().size() / 10) + 1) % 10); // 현재 보고 있는 페이지의 게시물 100개보다
																							// 마지막 게시물이 적다면 게시물만큼 보여줌
		}
	...
	public String boardListByType(@PathVariable("key") String continent, @PathVariable("id") Integer page, Model model,
		...
		if ((page / 10) * 10 + 10 < (boards.size() / 10) + 1) // 현제 보고있는 페이지보다 게시물이 100개 이상 있는 경우는 모두 보여줌
		{
			...
			model.addAttribute("last", ((boards.size() / 10) + 1) % 10); // 현재 보고 있는 페이지의 게시물 100개보다 마지막 게시물이 적다면 게시물만큼
																			// 보여줌
		}
	...
	public String ragneSelect(@PathVariable("key") String continent, @PathVariable("type") String boardType,
			@PathVariable("id") Integer page, Model model, HttpServletRequest request) {
...
//===================== 게시물 수정 ===============================
	...
	public String edit(@PathVariable("id") Long boardId, BoardDTO boardDTO, BindingResult bindingResult, Model model) {
		...
		if (!StringUtils.hasText(boardDTO.getPost_title())) {
			bindingResult.addError(
					new FieldError("dto", "post_title", boardDTO.getPost_title(), false, null, null, "제목은 필수입니다."));
		} // 제목 검사 ( 값을 입력했는지 검사 )

		if (!StringUtils.hasText(boardDTO.getPost_content())) {
			bindingResult.addError(
					new FieldError("dto", "post_content", boardDTO.getPost_content(), false, null, null, "내용은 필수입니다."));
		} // 내용 검사 ( 값을 입력했는지 검사 )
		...
		if (boardDTO.getWriteType() == null) {
			bindingResult.addError(
					new FieldError("dto", "writeType", boardDTO.getWriteType(), false, null, null, "게시판을 선택해주세요"));
		} // 게시판 선택했는지 검사
	...
	public void writeComment(Model model, SendDTO dto, HttpServletRequest request) {
		...
		reply.setReplyInsertDate(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))); // 댓글
																													// 작성
																													// 시간
	...
	public void addReplyComment(SendDTO dto, HttpServletRequest request) {
		...
		reply.setReplyInsertDate(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss")));
